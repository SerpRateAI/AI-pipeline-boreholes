...
class Microstructure:
    ...
    def calculate_polytope_fn(self):
        ...
        if Pn[-1] != 0.0:
            ...
            phi_n = Pn[-2]
        fn = (Pn - phi_n) / (phi - phi_n)
        self.polyfn_L = np.zeros(self.polytope_L.shape)
        self.polyfn_L[:, 1] = fn
        self.polyfn_L[:, 0] = self.polytope_L[:, 0]


def twoDCTimage2structure(input_image, par={'name': 'microstructure_from_image', 'begx': 10, 'begy': 10, 'nsampx': 251, 'nsampy': 251, 'edge_buffer': 10,
    ...
    min_samp_num = par['nsampx'] + 2 * par['edge_buffer']  # minimum number of samples required
    ...
    output_microstructure = Microstructure(im_dims, min(par['nsampx'], par['nsampy']))
    ix_beg = par['begx']  # index of window origin in x
    ix_end = ix_beg + (par['nsampx'])  # index of window end in x
    iy_beg = par['begy']  # index of window origin in y
    iy_end = iy_beg + (par['nsampy'])  # index of window end in y
